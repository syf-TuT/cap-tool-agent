...
| R08 | `docs/plans/2026-08-14-capsule-llm-step-only-runtime-design.md` | 单一逐动作循环、前向恢复约束与报告协议 |
...
| I06 | `capx/envs/trial.py` | Capsule 逐动作试验循环 |
| I07 | `capx/envs/trial.py` | `append_recovery` 语法、新观测与恢复代授权验证 |
| I08 | `capx/envs/trial.py` | 历史副作用单元禁止重放与账本更新 |
...
| C06 | 机器人副作用单元执行后，源代码补丁不会恢复物理前置状态 | R03“Problem”；R08“Stable Unit Lineage” | 用户设计事实 | 中 | 这是设计动机和系统约束，尚缺独立对照实验验证其性能影响 | “对已改变物理状态的动作，文本修补不能等价恢复其执行前状态。” |
...
| C09 | Capsule 在持久命名空间中执行模型选定的单元，并记录局部原语证据 | R01“Architecture”；R02“Trace Integration”；I03；I04；I06 | 用户方法事实 | 强 | 配置的执行粒度决定模型可选的单元动作集 | “运行时保留变量状态，并将原语调用证据绑定到当前源码单元。” |
| C10 | 历史副作用单元不能重放或补丁后再执行 | R03“No-Rerun Guard”；R08“Stable Unit Lineage”；I08 | 用户方法事实 | 强 | 无副作用计算单元仍可在单一逐动作循环中检查或修补 | “副作用账本把已执行物理动作转为只读历史。” |
...
| C15 | 重试和服务异常需要与算法结果分开报告 | R08“Telemetry”；I10；E02、E08 | 用户方法与实验事实 | 强 | 历史档案并非全部包含新 schema | “报告首次尝试、重试预算和基础设施失败，避免选择性结果掩盖真实成本。” |
