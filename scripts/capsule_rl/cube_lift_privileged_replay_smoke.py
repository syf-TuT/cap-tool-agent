...
CANONICAL_PROMPT = "\n".join(
    (
        "Your task is to pick up the red cube and lift it. Only the five high-level functions "
        "below are available:",
        ...
        "All quaternion values use WXYZ order. Do not access a raw environment object. Do not "
        "use low-level joint control. Return one complete executable Python program as code "
        "only, without Markdown code fences.",
    )
)
