...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("not_last", "guided"),
        ("zero_reward", "guided"),
        ("missing_trace", "guided"),
        ("base_success", "repair_attempts must contain exactly 0 attempts"),
    ],
)
def test_guided_member_must_satisfy_strict_seven_plus_one_contract(
    ...
    if case == "multiple":
        ...
        assembly = replace(
            ...
            base_results=(
                replace(
                    assembly.base_results[0],
                    outcome=ReplayOutcome.SUCCESS,
                    raw_reward=1.0,
                    binary_reward=1.0,
                    task_completed=True,
                ),
                *assembly.base_results[1:],
            ),
        )
