...
def test_capsule_trial_defaults_to_llm_step(monkeypatch):
    expected = object()

    def fake_llm_step(**kwargs):
        return expected

    monkeypatch.setattr(trial_module, "_run_capsule_llm_step_loop", fake_llm_step)
...
def test_capsule_trial_rejects_removed_legacy_mode():
    legacy_mode = "auto" + "_" + "forward"

    with pytest.raises(ValueError, match="Unsupported capsule_control_mode"):
        _run_capsule_trial(
            env=FakeCapsuleEnv(),
            trial=1,
            args=SimpleNamespace(),
            config={"capsule_control_mode": legacy_mode},
            initial_code="x = 1\n",
        )
